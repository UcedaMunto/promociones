...
x.envio_detalleCompra_lote.addEventListener("blur", myBlurFunction, true);          //Ejecuta una funcion cuando se quita seleccion del campo lote
...
function validarEnvioNew() {
    ...
    if(!document.getElementById('envio_dato').reportValidity()){
        ...
        if(fileValidation() != false){
            ...
            ruta = formularioCliente[0].action
            $.ajax({
...
function fileValidation(){
    ...
    if(filePath != ''){
        ...
        if(!allowedExtensions.exec(filePath)){
            alertify.error('Debe subir un archivo con extension .jpeg/.jpg/.png/.gif/.pdf/.heic');
            fileInput.value = '';
            document.getElementById("btnenvio").onclick = function(event) {
                event.preventDefault();
                validarEnvioNew();
